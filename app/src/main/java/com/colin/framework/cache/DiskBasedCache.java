...
public class DiskBasedCache implements Cache {
	...
	public synchronized void initialize() {
		if (!mRootDirectory.exists()) {
			if (!mRootDirectory.mkdirs()) {
				Log.e("DiskBasedCache", "Unable to create cache dir %s" + mRootDirectory.getAbsolutePath());
			}
			return;
		}
	...
	public synchronized void put(String key, Entry entry) {
		...
		try {
			...
			if (!success) {
				fos.close();
				Log.d("DiskbasedCached", "Failed to write header for " + file.getAbsolutePath());
		...
		if (!deleted) {
			Log.d("DiskbasedCached", "Could not clean up file " + file.getAbsolutePath());
		}
	}
	...
	public synchronized void remove(String key) {
		...
		removeEntry(key);
		if (!deleted) {
			// Log.d("Could not delete cache entry for key=%s, filename=%s",
	...
	private void pruneIfNeeded(int neededSpace) {
		...
		// if (Log.DEBUG) {
		// Log.v("pruned %d files, %d bytes, %d ms", prunedFiles,
